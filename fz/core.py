...
def fzr(
    ...
    # Campaign manifest (+ optional RO-Crate); traceability must never fail a run
    try:
        manifest = build_manifest(
            results_dir, model=model, calculators=calculators, input_path=input_path,
            input_variables=input_variables, results=final_results,
            start_time=campaign_start, end_time=utc_now(), interrupted=_interrupt_requested,
        )
        write_manifest(results_dir, manifest)
        if get_config().ro_crate:
            write_ro_crate(results_dir, manifest)
    except Exception as e:
        log_warning(f"⚠️  Could not write campaign manifest: {e}")
